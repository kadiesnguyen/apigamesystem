// src/games/slots/mahjongway/gridmodel.ts
...
export interface StepRoundPayload {
  ...
  aboveGrid: Cell[][];       // buffer phía trên (gần nhất r=0)
  winCells: Pos[];           // toàn bộ ô nằm trong line thắng (kể cả golden không bị clear)
  clearedCells: Pos[];       // tập con của winCells: ô bị loại khỏi lưới
  stepWin: number;
  multiplier: number;
  ...
  nextAboveGrid: Cell[][];   // buffer phía trên sau khi rơi xuống
...
export class GridModel {
  ...
  constructor(
    ...
    this.cols = cols;
    this.rows = rows;
    this.bufferRows = Math.max(0, MahjongWayConfig.RowsAbove ?? 0);
    this.topBufferReserve = Math.max(1, this.rows + this.bufferRows);
    this.payoutTable = payoutTable;
    this.scatterChance = scatterChance;
    this.goldenChance = goldenChance;
    this.noWinRate = noWinRate;
    this.symbolCount = MahjongWayConfig.SymbolNames?.length ?? 8;
    this.goldenReelSet = new Set(MahjongWayConfig.GoldenReelIndexes ?? []);
    this.debugEnabled = Boolean(MahjongWayConfig.DebugSpinLog);
  ...
  private ensureAllTopBuffersReady(minTarget?: number) {
    // Đảm bảo mỗi cột luôn có sẵn hàng đợi symbol đủ dài trước khi cascade xảy ra.
  ...
  private ensureTopBufferColumn(column: number, minTarget?: number) {
    // Seed lại queue cho một cột cụ thể; queue dài hơn RowsAbove để đủ symbol cho mọi trường hợp clear.
    if (this.topBufferReserve <= 0) return;
  ...
  private pullFromTopBuffer(column: number, count: number): Cell[] {
    // Lấy ra 'count' symbol từ đầu queue và ngay lập tức bổ sung lại phía sau để giữ queue ổn định.
    if (count <= 0) return [];
  ...
  /**
   * Quét toàn bộ lưới hiện tại để tìm line thắng.
   * Trả về:
   *   - tổng win theo bảng trả thưởng,
   *   - danh sách tất cả ô thuộc line thắng,
   *   - danh sách ô vàng sẽ flip sang wild (phục vụ hoạt ảnh).
   */
  private evaluateWins(grid: Cell[][], totalBet: number): {
    ...
    const baseBet = MahjongWayConfig.BaseBet ?? 20;
    ...
    for (let symbol = 0; symbol < this.symbolCount; symbol++) {
      // Gom lại các cột liên tiếp có ít nhất 1 matching symbol.
      const columns: Pos[][] = [];
      ...
      for (let c = 0; c < this.cols; c++) {
        ...
        for (let r = 0; r < this.rows; r++) {
          ...
          // Wild được tính như 1 symbol phụ trợ (trừ cột đầu tiên).
          const matchesSymbol =
            cell.idx === symbol || cell.isWild;
  ...
  /**
   * Chạy 1 lượt spin đầy đủ (bao gồm cascade) và chụp lại từng round để gửi client.
   * Luồng chính:
   *   1. Random fill lưới ban đầu (hoặc ép no-win).
   *   2. Lặp cascade: chụp grid, tính win, biến golden -> wild, clear ô thắng, collapse và refill.
   *   3. Lưu dữ liệu phục vụ client: grid/above trước-sau, flips, clearedCells, multiplier...
   */
  public spinWithCascadeAuthoritative(
    ...
    while (true) {
      // 1) Ảnh chụp đầu step (grid + queue phía trên) để serialize cho client.
      this.ensureAllTopBuffersReady();
      const gridStart = this.deepCopy(this.data);
      const aboveStart = this.snapshotAboveBuffer();

      // 2) Tính winCells trên gridStart theo luật 1024 ways
      ...
      // Chuyển các golden trong line thắng thành wild ngay tại ô hiện tại.
      ...
      for (const { c, r } of toClearNow) {
        ...
        if (goldenWildPositions.has(key)) {
          // Golden tiles that transformed into wilds stay on the board for the next cascade.
          continue;
        }
        const cell = this.data[c]?.[r];
        if (!cell) continue;
        // Đánh dấu idx = -1 để collapseGrid hiểu đây là ô bị clear.
        cell.idx = -1;
      ...
      if (MahjongWayConfig.Gravity === 'collapse') {
        this.collapseGrid();
      } else {
        this.refillOnlyCleared();
      }

      // Ảnh chụp sau khi clear + rơi để client dùng cho step kế tiếp.
  ...
  private collapseGrid() {
    for (let c = 0; c < this.cols; c++) {
      ...
      if (missing > 0) {
        // Ưu tiên rút symbol đã chuẩn bị sẵn trong queue; queue đã có đầy đủ dữ liệu để client sync.
      ...
      while (survivors.length < this.rows) {
  ...
  private logRoundDebug(payload: {
    ...
    console.log('   ▶️ Grid trước khi xử lý (Row 0 = hàng trên cùng):');
    ...
    console.log('   ⬇️ Grid sau khi clear + collapse (Row 0 = hàng trên cùng):');
  ...
  private renderGrid(grid: Cell[][], opts?: { highlight?: Set<string> }): string {
    ...
    const lines: string[] = [];
    for (let visualRow = 0; visualRow < this.rows; visualRow++) {
      const engineRow = this.rows - 1 - visualRow;
      const rowCells: string[] = [];
      for (let c = 0; c < this.cols; c++) {
        const key = `${c},${engineRow}`;
        const isMarked = highlight?.has(key) ?? false;
        rowCells.push(this.formatCell(grid[c]?.[engineRow], isMarked));
      }
      lines.push(`      Row ${visualRow} (engine r=${engineRow}): ${rowCells.join(' ')}`);
    }
    return lines.join('\n');
  }
  ...
  private renderAbove(buffer: Cell[][]): string {
    ...
    for (let r = 0; r < this.bufferRows; r++) {
      ...
      lines.push(`      Above ${r} (rơi sớm nhất ở chỉ số thấp): ${rowCells.join(' ')}`);
    }
    return lines.join('\n');
  }
  ...
  private describeCellLabel(cell: Cell): string {
    ...
    const name = MahjongWayConfig.SymbolNames?.[cell.idx] ?? `#${cell.idx}`;
